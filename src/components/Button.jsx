...
export default defineComponent({
  ...
  setup(props, { slots, emit }) {
    const { type, variant, size, squared, pill, href, iconOnly, srText, to } = props
    ...
    if (href) {
      return () => (
        <a href={!disabled.value ? href : null} class={classes.value} aria-disabled={disabled.value.toString()}>
      ...
      return () => (
        <RouterLink to={!disabled.value ? to : '#'} class={classes.value} aria-disabled={disabled.value.toString()}>
          {srText && <span class="sr-only">{srText}</span>}
          {slots.default?.({ iconSizeClasses })}
        </RouterLink>
      )
      ...
      return () => (
        <button onClick={handleClick} type={type} class={classes.value} disabled={disabled.value}>
          {srText && <span class="sr-only">{srText}</span>}
          {slots.default?.({ iconSizeClasses })}
        </button>
      )
